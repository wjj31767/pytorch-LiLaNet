...
def run(args):
    ...
    def _prepare_batch(batch, non_blocking=True):
        ...
        return (
            convert_tensor(
                distance, device=device, non_blocking=non_blocking), convert_tensor(
                reflectivity, device=device, non_blocking=non_blocking), convert_tensor(
                target, device=device, non_blocking=non_blocking))
    ...
    def run_validation(engine):
        pbar.log_message(
            "Start Validation - Epoch: [{}/{}]".format(engine.state.epoch, engine.state.max_epochs))
        ...
        iou_text = ', '.join(['{}: {:.1f}'.format(
            DENSE.classes[i + 1].name, v) for i, v in enumerate(iou.tolist())])
        pbar.log_message(
